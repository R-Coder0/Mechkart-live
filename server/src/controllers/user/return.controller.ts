        ...
        for (const it of so.items || []) {
            ...
            for (const ret of so.returns || []) {
                ...
                for (const rit of ret.items || []) {
                    const rpid = toStr(rit.productId);
                    const rvid = rit.variantId ? toStr(rit.variantId) : "";
                    const rck = rit.colorKey ? toStr(rit.colorKey).toLowerCase() : "";
                    const rkey = `${rpid}__${rvid}__${rck}`;

                    if (rkey === key) {
                        map.get(key).returnedQty += rit.qty;
                    }
                }
            }
        }
...
export const createReturnRequest = async (req: Request, res: Response) => {
    try {
        ...
        if (!bodyItems.length)
            return res.status(400).json({ message: "Select at least one item for return" });

        const itemMap = buildItemMap(order.subOrders);
        const vendorBuckets = new Map<string, any[]>();

        for (const r of bodyItems) {
            const pid = toStr(r.productId);
            const vid = r.variantId ? toStr(r.variantId) : "";
            ...
            if (reqQty > availableQty)
                return res.status(400).json({ message: "Return qty exceeds allowed qty" });
            ...
            vendorBuckets.get(soId)!.push({
                ...
                qty: reqQty,
                variantId: vid && Types.ObjectId.isValid(vid)
                    ? new Types.ObjectId(vid)
                    : null,
                colorKey: ck || null,
            });
        }
        ...
        if (pm === "COD") {
            const parsed = typeof req.body?.bankDetails === "string"
