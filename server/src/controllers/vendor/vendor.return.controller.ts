...
export const approveReturnByVendor = async (req: Request, res: Response) => {
    try {
        ...
        // Ensure this subOrder belongs to this vendor
        if (
            subOrder.ownerType !== "VENDOR" ||
            toStr(subOrder.vendorId) !== toStr(vendorId)
        ) {
        ...
        return res.json({ message: "Return approved successfully" });
