...
exports.adminCreateShiprocketShipment = void 0;
...
// helpers
const safeKey = (v) => String(v ?? "")
    .trim()
    .toUpperCase()
    .replace(/\s+/g, "-")
    .replace(/[^A-Z0-9\-]/g, "")
    .slice(0, 12);
const shortId = (v, len = 6) => {
    const s = String(v ?? "");
...
async function ensureVendorPickupOnShiprocket(vendorDoc) {
    const alias = vendorPickupAlias(vendorDoc?._id);
    // Try to avoid duplicate creates
    try {
        const list = await (0, shiprocket_service_1.shiprocketListPickupLocations)();
        const arr = list?.data || list?.pickup_address_list || list || [];
        if (Array.isArray(arr)) {
            const found = arr.find((x) => {
                const pl = toStr(x?.pickup_location ||
    ...
    // Create pickup
    try {
        await (0, shiprocket_service_1.shiprocketCreatePickupLocation)({
            pickup_location: alias,
            name: toStr(vendorDoc?.pickupAddress?.name) ||
        ...
        if (!isAlreadyExistsPickupError(e))
            throw e;
    }
    return alias;
}
...
function extractDimWeightFromProduct(p, variantId) {
...
const adminCreateShiprocketShipment = async (req, res) => {
    try {
        ...
        if (!deliveryPincode || deliveryPincode.length !== 6)
            return res.status(400).json({ message: "Invalid delivery pincode" });
        if (!deliveryPhone || deliveryPhone.length < 10)
            return res.status(400).json({ message: "Invalid delivery phone" });
        if (!deliveryCity || !deliveryState)
            return res.status(400).json({ message: "Delivery city/state missing" });
        ...
        const pendingSubOrders = subOrders.filter((so) => {
        ...
        const vendorIds = pendingSubOrders
            .map((so) => (so?.ownerType === "VENDOR" ? String(so.vendorId || "") : ""))
            .filter(Boolean);
        const vendors = vendorIds.length
            ? await Vendor_model_1.Vendor.find({ _id: { $in: vendorIds } }).select("email phone company pickupAddress").lean()
            : [];
        const vendorMap = new Map(vendors.map((v) => [String(v._id), v]));
        // preload products for dims
        const allItemRefs = pendingSubOrders.flatMap((so) => (Array.isArray(so.items) ? so.items : []));
        const productIds = Array.from(new Set(allItemRefs.map((it) => String(it.productId)).filter(Boolean)));
        const products = productIds.length
            ? await Product_model_1.Product.find({ _id: { $in: productIds } })
        ...
        for (const so of pendingSubOrders) {
            ...
            if (String(so.ownerType) === "VENDOR") {
                ...
                if (!vdoc)
                    throw new Error(`Vendor not found for subOrder vendorId=${String(so.vendorId)}`);
                pickup_location = await ensureVendorPickupOnShiprocket(vdoc);
            }
            // order_items (unique sku)
            const usedSkus = new Set();
            const order_items = soItems.map((it, idx) => {
            ...
            const sub_total = order_items.reduce((sum, it) => sum + Number(it.selling_price || 0) * Number(it.units || 1), 0);
            ...
            // ✅ serviceability
            let courier_company_id = null;
            const pickupPincode = String(so.ownerType) === "VENDOR"
            ...
            try {
                if (pickupPincode && pickupPincode.length === 6) {
                    const svc = await (0, shiprocket_service_1.shiprocketCheckServiceability)({
            ...
            const srOrderId = `${String(order.orderCode || order._id)}-${String(so.ownerType)}-${String(so.ownerType === "VENDOR" ? so.vendorId : "ADMIN").slice(-6)}`;
            const srOrderPayload = {
                order_id: srOrderId,
                order_date: (0, shiprocket_service_1.shiprocketFormatOrderDate)(new Date(order.createdAt || Date.now())),
                pickup_location,
            ...
            const srCreateResp = await (0, shiprocket_service_1.shiprocketCreateOrder)(srOrderPayload);
            ...
            if (courier_company_id) {
                awbResp = await (0, shiprocket_service_1.shiprocketGenerateAwb)({
                    shipment_id: Number(shipment_id),
                    courier_id: Number(courier_company_id),
                });
                awb =
                    firstNonEmptyString(awbResp?.awb_code, awbResp?.awb, awbResp?.response?.data?.awb_code, awbResp?.response?.data?.awb, awbResp?.data?.awb_code, awbResp?.data?.awb, awbResp?.response?.awb_code, awbResp?.response?.awb) || null;
                if (awb) {
                    labelResp = await (0, shiprocket_service_1.shiprocketGenerateLabel)({
                        shipment_id: [Number(shipment_id)],
                    });
            ...
            await Order_model_1.Order.updateOne({ _id: new mongoose_1.Types.ObjectId(orderId), "subOrders._id": so._id }, {
                $set: {
        ...
        if (createdResults.length) {
            await Order_model_1.Order.updateOne({ _id: new mongoose_1.Types.ObjectId(orderId) }, { $set: { status: "SHIPPED" } });
        }
...
exports.adminCreateShiprocketShipment = adminCreateShiprocketShipment;
