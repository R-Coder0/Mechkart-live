...
const generateNextProductId = async () => {
    const prefix = "MECH";
    const lastProduct = await Product_model_1.Product.findOne({})
        .sort({ createdAt: -1 })
        .select("productId")
        .lean();
    let nextNumber = 1;
...
const createProduct = async (req, res, next) => {
    try {
        const { title, description, features, featureImage, galleryImages, mrp, salePrice, variants, colors, // ✅ NEW
        baseStock, lowStockThreshold, categoryId, subCategoryId, } = req.body;
...
const getAllProducts = async (req, res, next) => {
    try {
        const { categoryId, subCategoryId, categorySlug, parentSlug, subSlug, active, lowStock, } = req.query;
        const filter = {};
...
const updateProduct = async (req, res, next) => {
    try {
        ...
        const { title, description, features, featureImage, // (string path from body - optional)
        galleryImages, mrp, salePrice, variants, colors, baseStock, lowStockThreshold, categoryId, subCategoryId, isActive, removeFeatureImage, // ✅ from frontend: "true"
         } = req.body;
        // -------- BASIC FIELDS --------
        if (title) {
            product.title = title;
