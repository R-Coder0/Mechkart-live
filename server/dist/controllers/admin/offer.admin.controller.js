...
exports.listOffers = exports.toggleOffer = exports.updateOffer = exports.createOffer = void 0;
...
const normalizeIds = (arr) => {
    ...
    return a
        .map((x) => String(x))
        .filter((x) => mongoose_1.Types.ObjectId.isValid(x))
        .map((x) => new mongoose_1.Types.ObjectId(x));
};
const upper = (s) => (typeof s === "string" ? s.trim().toUpperCase() : "");
...
const validateOfferPayload = (body) => {
    ...
    if (!["FLAT", "PERCENT"].includes(body.type))
    ...
    if (!["AUTO", "COUPON"].includes(body.mode))
        throw new Error("Invalid offer mode");
...
const createOffer = async (req, res, next) => {
    try {
        ...
        const offer = await Offer_model_1.Offer.create({
...
const updateOffer = async (req, res, next) => {
    try {
        ...
        if (patch.categoryIds !== undefined)
            patch.categoryIds = normalizeIds(patch.categoryIds);
        if (patch.subCategoryIds !== undefined)
            patch.subCategoryIds = normalizeIds(patch.subCategoryIds);
        if (patch.productIds !== undefined)
            patch.productIds = normalizeIds(patch.productIds);
        if (patch.value !== undefined)
            patch.value = toNumber(patch.value);
        if (patch.maxDiscountAmount !== undefined)
            patch.maxDiscountAmount = toNumber(patch.maxDiscountAmount);
        if (patch.globalUsageLimit !== undefined)
            patch.globalUsageLimit = toNumber(patch.globalUsageLimit);
        if (patch.perUserLimit !== undefined)
            patch.perUserLimit = toNumber(patch.perUserLimit);
        if (patch.priority !== undefined)
            patch.priority = toNumber(patch.priority);
        ...
        if (patch.mode === "COUPON") {
            ...
            if (patch.couponCode !== undefined)
                patch.couponCode = upper(patch.couponCode);
        }
        const offer = await Offer_model_1.Offer.findByIdAndUpdate(id, patch, { new: true });
        if (!offer)
...
const listOffers = async (req, res, next) => {
    try {
        const { q, mode, scope, isActive, page = "1", limit = "20" } = req.query;
        const filter = {};
        if (q)
        ...
        const [items, total] = await Promise.all([
            Offer_model_1.Offer.find(filter).sort({ createdAt: -1 }).skip((p - 1) * l).limit(l),
            Offer_model_1.Offer.countDocuments(filter),
        ]);
