...
exports.offerPreview = exports.getCheckoutSummary = void 0;
...
const oid = (v) => new mongoose_1.Types.ObjectId(String(v));
/** =========================
...
const getShippingMarkupForProduct = (product) => {
    // ✅ ONLY vendor products
    if (String(product?.ownerType) !== "VENDOR")
        return 0;
 ...
 * Offer helpers
 * ========================= */
function buildEligibleSetFromItems(appliedOffer, items) {
    const scope = String(appliedOffer?.scope || "SITE");
    if (scope === "SITE") {
        return new Set(items.map((it) => String(it.productId)));
    }
    if (scope === "PRODUCT") {
        const allowed = new Set((appliedOffer?.productIds || []).map((x) => String(x)));
        return new Set(items
            .filter((it) => allowed.has(String(it.productId)))
            .map((it) => String(it.productId)));
    }
    if (scope === "CATEGORY") {
        const allowed = new Set((appliedOffer?.categoryIds || []).map((x) => String(x)));
        return new Set(items
            .filter((it) => allowed.has(String(it?.product?.category)))
            .map((it) => String(it.productId)));
    }
    if (scope === "SUBCATEGORY") {
        const allowed = new Set((appliedOffer?.subCategoryIds || []).map((x) => String(x)));
        return new Set(items
...
function allocateDiscountToItems(items, eligibleSet, totalDiscount) {
...
function groupBySeller(items) {
    ...
    for (const it of items) {
        ...
        const vendorId = ownerType === "VENDOR" && (it.vendorId || it?.product?.vendorId)
...
const getCheckoutSummary = async (req, res) => {
    try {
        ...
        const couponCode = toStr(req.query?.couponCode || req.body?.couponCode) || "";
        const ownerKey = `u:${String(userId)}`;
        const cart = (await Cart_model_1.Cart.findOne({ ownerKey })) || (await Cart_model_1.Cart.findOne({ userId }));
        ...
        const productIds = Array.from(new Set(selectedItems.map((it) => String(it.productId))));
        const products = await Product_model_1.Product.find({
            _id: { $in: productIds },
            isActive: true,
        })
            .select("productId title slug featureImage galleryImages variants colors isActive mrp salePrice baseStock stock quantity category subCategory ownerType vendorId ship")
            .lean();
        const productMap = new Map(products.map((p) => [String(p._id), p]));
        // vendor names
        const vendorIds = Array.from(new Set(products
            .filter((p) => String(p.ownerType) === "VENDOR" && p.vendorId)
            .map((p) => String(p.vendorId))));
        const vendors = vendorIds.length
            ? await Vendor_model_1.Vendor.find({ _id: { $in: vendorIds } }).select("company.name").lean()
            : [];
        ...
        const items = selectedItems.map((it) => {
            ...
            let variant = null;
            if (hasVariants) {
                variant = (product.variants || []).find((v) => String(v._id) === String(it.variantId));
                if (!variant)
            ...
            const vendorId = ownerType === "VENDOR" && product.vendorId ? String(product.vendorId) : null;
        ...
        const subtotal = round2(items.reduce((s, i) => s + toNum(i.lineTotal, 0), 0));
        const mrpTotal = round2(items.reduce((s, i) => s + toNum(i.mrp, 0) * toNum(i.qty, 0), 0));
        ...
        const eligibleSet = appliedOffer ? buildEligibleSetFromItems(appliedOffer, items) : new Set();
 ...
 * OFFER PREVIEW
 * ========================= */
const offerPreview = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const subtotal = round2(items.reduce((s, it) => s + Number(it.lineTotal || 0), 0));
        const lines = items.map((it) => ({
            productId: oid(it.productId),
            qty: Number(it.qty || 0),
            salePrice: Number(it.salePrice || 0), // ✅ payable salePrice
