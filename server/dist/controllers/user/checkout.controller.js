...
exports.getCheckoutSummary = void 0;
...
const getCheckoutSummary = async (req, res) => {
    try {
        ...
        const cart = (await Cart_model_1.Cart.findOne({ ownerKey })) ||
            (await Cart_model_1.Cart.findOne({ userId })); // fallback for older carts
        ...
        const products = await Product_model_1.Product.find({
            ...
            .select("productId title slug featureImage galleryImages variants colors isActive mrp salePrice baseStock stock quantity")
            .lean();
        ...
        // 4) Totals
        const subtotal = items.reduce((s, i) => s + toNum(i.lineTotal, 0), 0);
        const mrpTotal = items.reduce((s, i) => s + toNum(i.mrp, 0) * toNum(i.qty, 0), 0);
