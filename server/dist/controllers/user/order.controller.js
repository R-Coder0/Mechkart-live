...
exports.downloadInvoicePdf = exports.getMyOrders = exports.getOrderById = exports.verifyRazorpayPayment = exports.createRazorpayOrder = exports.createCodOrder = void 0;
exports.getVendorCompanyName = getVendorCompanyName;
exports.getVendorSoldBy = getVendorSoldBy;
...
const offer_apply_service_1 = require("../../services/offer.apply.service");
...
const razorpay = RAZORPAY_KEY_ID && RAZORPAY_KEY_SECRET
    ? new razorpay_1.default({ key_id: RAZORPAY_KEY_ID, key_secret: RAZORPAY_KEY_SECRET })
    : null;
...
const getShippingMarkupForProduct = (p) => {
...
const computeVariantText = (p, variantId) => {
    const v = (p?.variants || []).find((x) => String(x._id) === String(variantId));
    if (!v)
        return "";
    return (toStr(v.label) ||
        toStr(v.comboText) ||
        toStr(v.size) ||
        toStr(v.weight) ||
        "");
...
function getVendorCompanyName(v) {
    return safeText(v?.company?.name) || "Vendor";
}
function getVendorSoldBy(v) {
...
// -----------------------------
function getVendorPickupSnapshot(v) {
 ...
 */
async function createOrderWithUniqueCode(createDoc) {
    let created = null;
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            const orderCode = await (0, orderCode_1.generateOrderCode)("MECH");
            created = await Order_model_1.Order.create({ ...createDoc, orderCode });
            break;
        }
        catch (err) {
    ...
    if (!created) {
        const e = new Error("Could not generate unique order code. Try again.");
        e.status = 500;
        throw e;
    }
    return created;
 ...
 */
async function buildCheckoutSnapshot(req, userId) {
    ...
    if (!contactName || !contactPhone) {
        const e = new Error("Contact name and phone are required");
        e.status = 400;
        throw e;
    }
    if (!mongoose_1.Types.ObjectId.isValid(addressId)) {
        const e = new Error("Valid addressId is required");
        e.status = 400;
        throw e;
    }
    ...
    const cart = (await Cart_model_1.Cart.findOne({ ownerKey })) ||
        (await Cart_model_1.Cart.findOne({ userId }));
    ...
    if (!selectedItems.length) {
        const e = new Error("No items selected for checkout");
        e.status = 400;
        throw e;
    }
    // Products
    ...
    const products = await Product_model_1.Product.find({
        ...
        .select("title productId variants colors galleryImages featureImage isActive mrp salePrice baseStock stock quantity vendorId vendorName ownerType ship  ")
        .lean();
    ...
    for (const it of selectedItems) {
        ...
        if (!p) {
            const e = new Error("Some products are unavailable now");
            e.status = 409;
            throw e;
        }
        const qty = Number(it.qty || 0);
        if (!qty || qty <= 0) {
            const e = new Error("Invalid quantity in cart");
            e.status = 409;
            throw e;
        }
        ...
        if (hasVariants) {
            ...
            if (!v) {
                const e = new Error("Selected variant no longer exists");
                e.status = 409;
                throw e;
            }
            const available = Number(v.quantity ?? v.stock ?? 0);
            if (available <= 0) {
                const e = new Error("Variant out of stock");
                e.status = 409;
                throw e;
            }
            if (qty > available) {
                const e = new Error("Quantity exceeds available stock");
                e.status = 409;
    ...
    const vendorIds = Array.from(new Set(products
        .map((p) => (p?.vendorId ? String(p.vendorId) : ""))
        .filter(Boolean)));
    const vendors = vendorIds.length
        ? await Vendor_model_1.Vendor.find({ _id: { $in: vendorIds } })
    ...
    const orderItems = selectedItems.map((it) => {
        ...
        const ownerType = vendorId ? "VENDOR" : "ADMIN";
        const vdoc = vendorId ? vendorMap.get(String(vendorId)) : null;
        const soldBy = ownerType === "VENDOR"
        ...
        return {
            ...
            ownerType,
            ship: shipSnap,
            vendorId: vendorId ? new mongoose_1.Types.ObjectId(vendorId) : null,
            soldBy,
            offerDiscount: 0,
    ...
    const offerResult = await (0, offer_apply_service_1.validateAndComputeOffer)({
    ...
    if (couponCode && !offerResult.ok) {
        const e = new Error(offerResult.reason || "Invalid coupon");
        e.status = 400;
        throw e;
    }
    ...
    for (const it of orderItems) {
        const key = it.ownerType === "VENDOR" && it.vendorId ? String(it.vendorId) : "ADMIN";
        if (!groups.has(key)) {
            const isVendor = key !== "ADMIN";
            const vdoc = isVendor ? vendorMap.get(String(key)) : null;
            ...
            groups.set(key, {
                ...
                vendorId: isVendor ? new mongoose_1.Types.ObjectId(String(key)) : null,
    ...
    const subOrders = Array.from(groups.values()).map((g) => {
        const subTotal = g.items.reduce((sum, it) => sum + Number(it.finalLineTotal || 0), 0);
        const shipping = 0;
...
const createCodOrder = async (req, res) => {
    try {
        ...
        if (snap.offerResult.ok && snap.offerResult.appliedOffer && snap.totals.discount > 0) {
            await (0, offer_apply_service_1.recordOfferRedemption)({
                offerId: snap.offerResult.appliedOffer._id,
                userId: new mongoose_1.Types.ObjectId(userId),
                orderId: new mongoose_1.Types.ObjectId(String(order._id)),
        ...
        // ✅ Emails (fail-safe)
        try {
            await (0, email_service_1.sendOrderPlacedCustomerEmail)(order);
            await (0, email_service_1.sendOrderPlacedOwnerEmail)(order);
        }
        catch (e) {
            console.error("Order placed email failed:", e?.message || e);
        }
        // ✅ Make selected address default
        for (const a of snap.user.addresses)
            a.isDefault = false;
        snap.user.addresses.id(String(req.body?.addressId)).isDefault = true;
        await snap.user.save();
        // ✅ Remove selected cart lines
        const selectedIds = new Set(snap.selectedItems.map((it) => String(it._id)));
        snap.cart.items = snap.cart.items.filter((it) => !selectedIds.has(String(it._id)));
...
const createRazorpayOrder = async (req, res) => {
    try {
        ...
        await Order_model_1.Order.updateOne({ _id: order._id }, {
            $set: {
        ...
        try {
            ...
            if (cart?.items?.length) {
                const used = order.items || [];
                cart.items = cart.items.filter((ci) => {
                    return !used.some((oi) => String(ci.productId) === String(oi.productId) &&
...
const verifyRazorpayPayment = async (req, res) => {
    try {
        ...
        const order = await Order_model_1.Order.findOne({ _id: orderId, userId: new mongoose_1.Types.ObjectId(userId) });
        if (!order)
            return res.status(404).json({ message: "Order not found" });
        if (order.paymentMethod !== "ONLINE") {
            return res.status(400).json({ message: "This order is not an ONLINE order" });
        }
        const savedRpOrderId = toStr(order?.pg?.orderId);
        ...
        if (order?.appliedOffer?.offerId && Number(order?.totals?.discount || 0) > 0) {
            try {
                await (0, offer_apply_service_1.recordOfferRedemption)({
                    offerId: new mongoose_1.Types.ObjectId(String(order.appliedOffer.offerId)),
                    userId: new mongoose_1.Types.ObjectId(userId),
                    orderId: new mongoose_1.Types.ObjectId(String(order._id)),
                    couponCode: order?.appliedOffer?.couponCode || null,
                    discountAmount: Number(order?.totals?.discount || 0),
                });
            }
            catch (e) {
        ...
        try {
            const fresh = await Order_model_1.Order.findById(order._id);
            if (fresh) {
                await (0, email_service_1.sendOrderPlacedCustomerEmail)(fresh);
                await (0, email_service_1.sendOrderPlacedOwnerEmail)(fresh);
            }
        }
        catch (e) {
            console.error("Order placed email failed:", e?.message || e);
...
const downloadInvoicePdf = async (req, res) => {
    try {
        ...
        const invoiceNumber = `MECH${createdAt.getFullYear()}${pad(createdAt.getMonth() + 1, 2)}${pad(createdAt.getDate(), 2)}-${String(order._id).slice(-6).toUpperCase()}`;
        ...
        for (const it of items) {
            ...
            const gross = mrp * qty;
            const lineTotal = Number(it.finalLineTotal ?? (sale * qty));
