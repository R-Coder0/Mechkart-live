...
exports.vendorUpdateMyProduct = exports.vendorGetMyProductById = exports.vendorListMyProducts = exports.vendorCreateProduct = void 0;
...
const calcLowStockFlag = (totalStock, threshold) => totalStock <= Number(threshold || 0);
 ...
 */
const vendorCreateProduct = async (req, res, next) => {
    try {
        const vendorIdRaw = req?.vendor?._id;
        const vendorId = mongoose_1.Types.ObjectId.isValid(String(vendorIdRaw)) ? new mongoose_1.Types.ObjectId(String(vendorIdRaw)) : null;
        if (!vendorId)
            return res.status(401).json({ message: "Unauthorized" });
        const { title, description, features, featureImage, galleryImages, mrp, salePrice, variants, colors, baseStock, lowStockThreshold, categoryId, subCategoryId, } = req.body;
        ...
        const existing = await Product_model_1.Product.findOne({ slug }).select("_id");
        ...
        const rawShipLength = req.body?.shipLengthCm ??
            req.body?.["ship[lengthCm]"] ??
            req.body?.ship?.lengthCm;
        const rawShipBreadth = req.body?.shipBreadthCm ??
            req.body?.["ship[breadthCm]"] ??
            req.body?.ship?.breadthCm;
        const rawShipHeight = req.body?.shipHeightCm ??
            req.body?.["ship[heightCm]"] ??
            req.body?.ship?.heightCm;
        const rawShipWeight = req.body?.shipWeightKg ??
            req.body?.["ship[weightKg]"] ??
            req.body?.ship?.weightKg;
        const ship = {
...
exports.vendorCreateProduct = vendorCreateProduct;
/**
 * VENDOR: LIST MY PRODUCTS
 * GET /api/vendors/products
 */
const vendorListMyProducts = async (req, res, next) => {
    try {
        const vendorIdRaw = req?.vendor?._id;
        const vendorId = mongoose_1.Types.ObjectId.isValid(String(vendorIdRaw)) ? new mongoose_1.Types.ObjectId(String(vendorIdRaw)) : null;
        if (!vendorId)
            return res.status(401).json({ message: "Unauthorized" });
        const { approvalStatus, active, lowStock, q } = req.query;
        const filter = {
            ownerType: "VENDOR",
            vendorId,
        };
...
exports.vendorListMyProducts = vendorListMyProducts;
/**
 * VENDOR: GET SINGLE MY PRODUCT
 * GET /api/vendors/products/:id
 */
const vendorGetMyProductById = async (req, res, next) => {
    try {
        const vendorIdRaw = req?.vendor?._id;
        const vendorId = mongoose_1.Types.ObjectId.isValid(String(vendorIdRaw)) ? new mongoose_1.Types.ObjectId(String(vendorIdRaw)) : null;
        if (!vendorId)
...
exports.vendorGetMyProductById = vendorGetMyProductById;
/**
 * VENDOR: UPDATE MY PRODUCT
 * PUT /api/vendors/products/:id
 * Rule: vendor edit => approvalStatus back to PENDING
 */
const vendorUpdateMyProduct = async (req, res, next) => {
    try {
        const vendorIdRaw = req?.vendor?._id;
        const vendorId = mongoose_1.Types.ObjectId.isValid(String(vendorIdRaw)) ? new mongoose_1.Types.ObjectId(String(vendorIdRaw)) : null;
        if (!vendorId)
        ...
        if (!product)
            return res.status(404).json({ message: "Product not found" });
        const { title, description, features, featureImage, galleryImages, mrp, salePrice, variants, colors, baseStock, lowStockThreshold, categoryId, subCategoryId, isActive, removeFeatureImage, shipLengthCm, shipBreadthCm, shipHeightCm, shipWeightKg, } = req.body;
        // TITLE + SLUG (avoid collisions)
        if (title) {
            const newSlug = makeSlug(title);
            const exists = await Product_model_1.Product.findOne({
                slug: newSlug,
                _id: { $ne: product._id },
            }).select("_id");
            product.title = title;
...
exports.vendorUpdateMyProduct = vendorUpdateMyProduct;
