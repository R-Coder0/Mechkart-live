...
const shouldApplyShippingMarkup = (req) => {
    const anyReq = req;
    if (anyReq?.vendor?._id)
        return false; // vendor panel never
 ...
 */
const enrichCartLean = async (req, cartLean) => {
    if (!cartLean)
        return null;
    ...
    const products = await Product_model_1.Product.find({
        ...
        .select("title slug featureImage galleryImages variants colors isActive ownerType vendorId approvalStatus ship mrp salePrice")
