...
exports.sendOrderPlacedCustomerEmail = sendOrderPlacedCustomerEmail;
exports.sendOrderPlacedOwnerEmail = sendOrderPlacedOwnerEmail;
...
function assertEnv() {
    ...
    if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
        throw new Error("Email config missing. Required: SMTP_HOST, SMTP_USER, SMTP_PASS (and SMTP_PORT optional).");
    }
    if (!SMTP_FROM) {
 ...
 */
async function sendOrderPlacedCustomerEmail(order) {
    if (!EMAIL_ENABLED)
        return { skipped: true };
 ...
 */
async function sendOrderPlacedOwnerEmail(order) {
    if (!EMAIL_ENABLED)
        return { skipped: true };
    const to = safeTrim(OWNER_EMAIL);
    if (!to)
        return { skipped: true, reason: "OWNER_EMAIL missing" };
    const orderCode = escapeHtml(order.orderCode || "—");
