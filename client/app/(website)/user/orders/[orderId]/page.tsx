...
export default function WebsiteUserOrderDetailsPage() {
  ...
  }, [subOrders.length, computedSubOrders]);

  const headerSubtitle = useMemo(() => {
    if (subOrders.length > 1) {
      const names = subOrders.map((s: any) => s.soldBy).filter(Boolean);
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="border bg-white overflow-hidden">
            ...
            <div className="p-5 flex gap-4 items-start">
              <div className="flex-1">
                ...
                <div className="mt-1 text-xs text-gray-500">{totalItemsCount} item(s)</div>
              </div>
                  ...
                  return (
                    <div key={so._id} className="border">
                      <div className="border-b px-4 py-3 text-sm font-semibold text-gray-900">
                        Sold by: {so.soldBy} {so.vendorName ? <span className="text-xs text-gray-500">({so.vendorName})</span> : null}
                      </div>

                      {!sh ? (
                        <div className="px-4 py-3 text-sm text-gray-600">Shipment not created yet.</div>
                      ) : (
                        <>
                          ...
                          {Array.isArray(events) && events.length ? (
                            <div className="border-t p-4">
                              ...
                              {events.length > 6 ? <div className="mt-3 text-xs text-gray-500">Showing latest 6 updates.</div> : null}
                            </div>
                          ) : null}
                        </>
                      )}
              ...
              <div className="mt-3 border p-4 text-sm">
                ...
                {returnReq?.bankDetails ? (
                  <div className="mt-3 border p-3 text-xs text-gray-700">
                    ...
                    <div>
                      <span className="font-semibold">A/C Holder:</span> {returnReq.bankDetails?.accountHolderName || "—"}
                    </div>
                    <div>
                      <span className="font-semibold">A/C No:</span> {returnReq.bankDetails?.accountNumber || "—"}
                    </div>
                    <div>
                      <span className="font-semibold">IFSC:</span> {returnReq.bankDetails?.ifsc || "—"}
                    </div>
                ...
                {returnReq?.requestedAt ? <div className="mt-2 text-[11px] text-gray-500">Requested: {fmtDateTime(returnReq.requestedAt)}</div> : null}
              </div>
            ) : (
              ...
              {computedSubOrders.map((so: any) => (
                <div key={so._id} className="border p-4">
                  ...
                  <div className="mt-4 space-y-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              {x.productCode ? <div className="mt-0.5 text-[11px] font-semibold text-gray-500">Code: {x.productCode}</div> : null}
                    ...
                    {!so.__uiItems.length ? <div className="text-sm text-gray-600">No items in this sub-order.</div> : null}
                  </div>
                </div>
              ))}

              {!computedSubOrders.length ? <div className="border bg-white p-5 text-sm text-gray-700">No items found.</div> : null}
            </div>
          </div>
          ...
          <div className="border bg-white p-5">
            ...
            <div className="mt-3 border px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">Did you find this page helpful?</div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="border bg-white p-5">
            ...
            <div className="mt-4 border p-4">
              <div className="text-sm font-semibold text-gray-900">{order?.address?.addressType ? String(order.address.addressType) : "Home"}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
          <div className="w-full max-w-lg border bg-white max-h-[90vh] overflow-hidden">
            <div className="border-b px-5 py-4">
              ...
              <div className="mt-1 text-xs text-gray-500">Order: {order?.orderCode || String(order?._id).slice(-8)}</div>
            ...
            <div className="p-5 space-y-4 overflow-y-auto max-h-[calc(90vh-120px)]">
              {rrErr ? <div className="border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{rrErr}</div> : null}
              ...
              <div className="border p-4">
                ...
                <div className="mt-3 space-y-4">
                  {computedSubOrders.map((so: any) => (
                    <div key={so._id} className="border p-3">
                      <div className="text-xs font-bold text-gray-900 mb-2">Sold by: {so.soldBy}</div>
                ...
                <div className="mt-2 text-[11px] text-gray-500">Tip: Select at least one item for partial return.</div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button type="button" disabled={rrBusy} onClick={() => setRrOpen(false)} className="border px-4 py-2 text-sm font-semibold hover:bg-gray-50 disabled:opacity-60">
                  Cancel
                </button>

                <button type="button" disabled={rrBusy} onClick={submitReturnRequest} className="bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-black disabled:opacity-60">
                  {rrBusy ? "Submitting…" : "Submit"}
                </button>
              </div>

              <div className="text-[11px] text-gray-500">Note: Images are optional. For COD refunds, bank details are required.</div>
            </div>
          </div>
        </div>
      ) : null}
