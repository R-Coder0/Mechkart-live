...
export default function WebsiteUserOrdersPage() {
  ...
  const load = async (query: string) => {
    try {
      ...
      // ✅ Assuming your endpoint exists like admin: GET /users/orders
      // If your backend uses different route, just change this URL.
      ...
      // support shapes: {items: []} or [] directly
  ...
  const filtered = useMemo(() => {
    ...
    return (orders || []).filter((o: any) => {
      ...
      return code.includes(s) || id.includes(s) || title.includes(s);
    });
  }, [orders, search]);

  const onSearch = () => {
    // UI-side filter only (fast)
    setSearch(q);
    // If you want server search also, uncomment:
    // load(q);
  };
      ...
      <div className="mt-5 space-y-4">
        {loading ? (
          ...
          filtered.map((o: any) => {
            ...
            // If backend populates productId, it will be at first.productId
            const product = first?.productId || first?.product || null;

            const title = String(product?.title || first?.title || "Product");
            const color = first?.colorKey ? String(first.colorKey) : "";
            const variantText = getVariantText(product, first?.variantId);

            const imgPath = resolveOrderItemImage(product, first?.variantId, first?.colorKey, first?.image);
            const img = resolveImageUrl(imgPath);

            const total = Number(o?.totals?.subtotal ?? o?.totalAmount ?? 0);
            ...
            if (status === "SHIPPED") {
              rightSub = "Your item has been shipped.";
            }
            if (status === "DELIVERED") {
            ...
            return (
              <Link
                key={String(o?._id)}
                href={`/website/user/orders/${String(o._id)}`}
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-gray-600">
                        ...
                        {color && variantText ? <span className="mx-2">•</span> : null}
                        {variantText ? <span>Variant: {variantText}</span> : null}
                      </div>

                      {/* Order code (optional small) */}
                      {o?.orderCode ? (
                        <div className="mt-1 text-[11px] text-gray-500">Order: {o.orderCode}</div>
                      ) : null}
                    </div>
                  </div>
          ...
          <div className=" border bg-white p-8 text-sm text-gray-700">
            No orders found.
          </div>
        )}
      </div>
